...
@AndroidEntryPoint
class FindMatchActivity : AppCompatActivity(), MatchAdapter.OnItemClickListener {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    val adapter = MatchAdapter (this)//{ item -> viewModel.selectItem(item) }
    ...
    getRemainingTotal(viewModel.matchItems.value.first().total)
  }
  
  private fun getRemainingTotal(minusOfSelectedItemTotal: Float = 0f) {
    val remainingTotal = intent.getFloatExtra(TARGET_MATCH_VALUE, DEFAULT_VALUE)
    viewModel.setRemainingTotalWhenStart(remainingTotal - minusOfSelectedItemTotal)
  }
  ...
  override fun onItemClick(item: MatchItem) {
    getRemainingTotal(item.total)
  }
